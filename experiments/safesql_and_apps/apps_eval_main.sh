...
bash run_response_generation_for_all_ckpts.sh $EXP_DIR "apps-code" "True" $DATA_ROOT
cd ..

# 3. Evaluate the responses for all checkpoints and the corresponding base model (aka ckpt "global_step_0")
cd result_evaluation
bash run_eval_for_all_ckpts.sh $EXP_DIR "apps-code" "mypy_typecheck" $DATA_ROOT
bash run_eval_for_all_ckpts.sh $EXP_DIR "apps-code" "leetcode_correctness" $DATA_ROOT
cd ..
